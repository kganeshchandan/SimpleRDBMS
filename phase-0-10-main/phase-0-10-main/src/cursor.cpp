...
vector<int> Cursor::getNext()
{
    ...
    this->pagePointer++;
    if(result.empty()){
        if     (tableCatalogue.isTable(this->tableName))   tableCatalogue.getTable(this->tableName)->getNextPage(this);
        else if(matrixCatalogue.isMatrix(this->tableName)) matrixCatalogue.getMatrix(this->tableName)->getNextPage(this);
